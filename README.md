...
- `ASTRAGAL_SANITY_PREVIEW_SECRET` can be any random string (but avoid spaces), like `MY_SECRET` - this is used for [the Preview Mode](https://nextjs.org/docs/advanced-features/preview-mode).
- `NEXT_PUBLIC_ASTRAGAL_SANITY_PROJECT_ID`: Get the `projectId` value from the `sanity.json` file created in step 2.
- `ASTRAGAL_SANITY_API_TOKEN`: Copy the API token generated in the previous step.

Your `.env` file should look like this:

```bash
ASTRAGAL_SANITY_PREVIEW_SECRET=...
NEXT_PUBLIC_ASTRAGAL_SANITY_PROJECT_ID=...
ASTRAGAL_SANITY_API_TOKEN=...
```
...
```js
const previewSecret = "MY_SECRET" // Copy the string you used for ASTRAGAL_SANITY_PREVIEW_SECRET
...
```
now secrets add ASTRAGAL_SANITY_PREVIEW_SECRET <ASTRAGAL_SANITY_PREVIEW_SECRET>
now secrets add ASTRAGAL_SANITY_API_TOKEN <ASTRAGAL_SANITY_API_TOKEN>
now secrets add NEXT_PUBLIC_ASTRAGAL_SANITY_PROJECT_ID <NEXT_PUBLIC_ASTRAGAL_SANITY_PROJECT_ID>
```
